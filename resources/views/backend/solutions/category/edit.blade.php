                                    ...
                                    <form class="row g-3 needs-validation custom-input" novalidate action="{{ route('manage-category.update', $banner_details->id) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-control" id="solution_type" name="solution_type" required>
                                                <option value="">-- Select Solutions Type --</option>
                                                @foreach($applications as $application)
                                                    <option value="{{ $application->id }}" {{ $banner_details->solution_id == $application->id ? 'selected' : '' }}>
                                                        {{ $application->solution_type }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            <div class="invalid-feedback">Please select an Solutions Type.</div>
                                        </div>
